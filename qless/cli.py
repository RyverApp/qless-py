"""Command-line entrypoint for running qless workers"""
...
def main():
    """Parse arguments and run a qless worker"""
    parser = argparse.ArgumentParser(description='Run qless workers.')

    # Options specific to the client we're making
    parser.add_argument('--host', dest='host', default='redis://localhost:6379', help='The redis:// url to connect to')
    parser.add_argument('-n', '--name', default=None, type=str, help='The hostname to identify your worker as')

    # Options that we consume in this binary
    parser.add_argument('-p', '--path', action='append', default=[], help='Path(s) to include when loading jobs')
    parser.add_argument('-v', '--verbose', default=False, action='store_true', help='Be extra talkative')
    parser.add_argument('-l', '--logger', default=False, type=str, help='Log out to this file')
    parser.add_argument('-m', '--import', action='append', default=[], help='The modules to preemptively import')
    ...
    parser.add_argument(
        '-w', '--workers', default=0, type=int, help='How many processes to run. Set to 0 to use all available cores'
    )
    parser.add_argument('-q', '--queue', action='append', default=[], help='The queues to pull work from')
    ...
    parser.add_argument('-i', '--interval', default=60, type=int, help='The polling interval')
    parser.add_argument(
        '-r',
        '--resume',
        default=False,
    ...
    for module in getattr(args, 'import'):
        try:
            logger.info(f'Loaded {__import__(module)!r}')
    ...
    if args.greenlets:
        kwargs.update({'klass': 'qless.workers.greenlet.GeventWorker', 'greenlets': args.greenlets})
