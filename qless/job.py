...
class BaseJob:
    ...
    def __setattr__(self, key, value):
        if key == 'priority':
            return self.client('priority', self.jid, value) and object.__setattr__(self, key, value)
        else:
...
class Job(BaseJob):
    ...
    def __repr__(self):
        return f'<{self.klass_name} {self.jid}>'
    ...
    def process(self):
        ...
        your class."""
        try:
            name = self.queue_name if hasattr(self.klass, self.queue_name) else 'process'
            method = getattr(self.klass, name, None)
        except Exception as exc:  # noqa: BLE001
            # We failed to import the module containing this class
            logger.exception(f'Failed to import {self.klass_name}')
            return self.fail(self.queue_name + '-' + exc.__class__.__name__, f'Failed to import {self.klass_name}')

        if method:
            if isinstance(inspect.getattr_static(self.klass, name, None), staticmethod):
                try:
                    logger.info(f'Processing {self.jid} in {self.queue_name}')
                    method(self)
                    logger.info(f'Completed {self.jid} in {self.queue_name}')
                except Exception as exc:  # noqa: BLE001
                    # Make error type based on exception type
                    logger.exception(f'Failed {self.jid} in {self.queue_name}: {method!r}')
                    self.fail(self.queue_name + '-' + exc.__class__.__name__, traceback.format_exc())
            else:
                # Or fail with a message to that effect
                logger.error(f'Failed {self.jid} in {self.queue_name} : {method!r} is not static')
                self.fail(self.queue_name + '-method-type', repr(method) + ' is not static')
        else:
            # Or fail with a message to that effect
            logger.error(f'Failed {self.jid} : {self.klass_name} is missing a method "{self.queue_name}" or "process"')
            self.fail(
    ...
    def move(self, queue, delay=0, depends=None):
        ...
        logger.info(f'Moving {self.jid} to {queue} from {self.queue_name}')
        return self.client(
            'put',
    ...
    def complete(self, nextq=None, delay=None, depends=None, result_data=None):
        ...
        dependencies"""
        if nextq:
            logger.info(f'Advancing {self.jid} to {nextq} from {self.queue_name}')
            return (
                self.client(
                    'complete',
                    self.jid,
            ...
            logger.info(f'Completing {self.jid}')
            return (
                self.client(
                    'complete',
                    self.jid,
    ...
    def heartbeat(self):
        ...
        user data."""
        logger.debug(f'Heartbeating {self.jid} (ttl = {self.ttl})')
        try:
        ...
        logger.debug(f'Heartbeated {self.jid} (ttl = {self.ttl})')
    ...
    def retry(self, delay=0):
        ...
        return self.client('retry', self.jid, self.queue_name, self.worker_name, delay)
...
class RecurringJob(BaseJob):
    ...
    def __setattr__(self, key, value):
        if key in ('priority', 'retries', 'interval'):
            return self.client('recur.update', self.jid, key, value) and object.__setattr__(self, key, value)
        if key == 'data':
            return self.client('recur.update', self.jid, key, json.dumps(value)) and object.__setattr__(
                self, 'data', value
            )
        if key == 'klass':
