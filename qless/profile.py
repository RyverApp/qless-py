...
class Profiler:
    ...
    def pretty(timings, label):
        """Print timing stats"""
        results = [(sum(values), len(values), key) for key, values in list(timings.items())]
        print(label)
        print('=' * 65)
        print(f'{"Command":>20} => {"Average":>13} | {"# Calls":>8} | {"Total time":>13}')
        print('-' * 65)
        for total, length, key in sorted(results, reverse=True):
            print(f'{key:>20} => {float(total) / length:>10.5f} us | {length:>8} | {total:>10} us')
    ...
    def stop(self):
        ...
        current = {'name': None, 'accumulated': defaultdict(list)}
        for _, _, duration, request in logs:
            ...
            if 'eval' in command.lower():
                ...
                self._timings[f'qless-{subcommand}'].append(duration)
                if current['name']:
    ...
    def display(self):
        ...
        for key, value in list(self._commands.items()):
            self.pretty(value, f'Qless "{key}" Command')
            print()
