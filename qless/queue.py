...
class Queue:
    ...
    def __getattr__(self, key):
        ...
        if key == 'heartbeat':
            ...
            return int(conf.get(self.name + '-heartbeat', conf.get('heartbeat', 60)))
        raise AttributeError(f'qless.Queue has no attribute {key}')
    ...
    def pop(self, count=None):
        ...
        results = [
            Job(self.client, **job) for job in json.loads(self.client('pop', self.name, self.worker_name, count or 1))
        ]
        if count == None:
    ...
    def peek(self, count=None):
        ...
        items"""
        results = [Job(self.client, **rec) for rec in json.loads(self.client('peek', self.name, count or 1))]
        if count == None:
            return (len(results) and results[0]) or None
        return results
