...
class TestJob(TestQless):
    ...
    def test_attributes(self):
        ...
        self.client.queues['foo'].put('Foo', {'whiz': 'bang'}, jid='jid', tags=['foo'], retries=3)
    ...
    def test_depend_undepend(self):
        ...
        self.assertEqual(sorted(self.client.jobs['c'].dependencies), ['a'])
        self.client.jobs['c'].depend('b')
        self.assertEqual(sorted(self.client.jobs['c'].dependencies), ['a', 'b'])
        self.client.jobs['c'].undepend('a')
        self.assertEqual(sorted(self.client.jobs['c'].dependencies), ['b'])
...
class TestRecurring(TestQless):
    def test_attributes(self):
        ...
        self.client.queues['foo'].recur('Foo', {'whiz': 'bang'}, 60, jid='jid', tags=['foo'], retries=3)
        job = self.client.jobs['jid']
        atts = ['data', 'jid', 'priority', 'klass_name', 'queue_name', 'tags', 'retries', 'interval', 'count']
        values = [getattr(job, att) for att in atts]
        self.assertEqual(
            dict(list(zip(atts, values))),
            {
